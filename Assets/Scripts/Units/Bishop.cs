...
class Bishop : BaseUnit
{
    ...
    public override void removeBlockedTiles()
    {
        foreach (Tile tile in possibleLocationTiles.Values.ToList<Tile>())
        {
            ...
            {
                float horizontalDistance = tile.transform.position.x - this.transform.position.x; // positive if the target is to the right of the selected piece, negative otherwise 2
                float verticalDistance = tile.transform.position.y - this.transform.position.y; // positive if the target is to the north(up) of the selected piece, negative otherwise -2
                for (int i = 1; i < 7; i++)
                { // (2,-2), 3,-3, 4,-4, ... 
